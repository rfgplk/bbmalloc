...
__addr_readable(const void *p) noexcept
{
  if( p == nullptr )
    return false;
  if( (reinterpret_cast<umax_t>(p) & (sizeof(void *) - 1)) != 0 )
    return false;     // a real fp is word-aligned
  unsigned char v = 0;
  ...
  (void)v;
  if( !micron::port::addr_readable(reinterpret_cast<void *>(lo)) )
    return false;
  if( hi != lo && !micron::port::addr_readable(reinterpret_cast<void *>(hi)) )
    return false;
  return true;
}
...
__print_stack()
{
  ...
  for( int i = 0; i < max_frames && fp; ++i ) {
    ...
    if( reinterpret_cast<umax_t>(next_fp) <= reinterpret_cast<umax_t>(fp) )
      break;
    if( reinterpret_cast<umax_t>(next_fp) - base > stack_window )
      break;
    buffer[n++] = next_ret;
